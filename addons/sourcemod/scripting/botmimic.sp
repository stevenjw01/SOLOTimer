...
public OnPluginStart()
{
	...
	g_hCVOriginSnapshotInterval = CreateConVar("sm_botmimic_snapshotinterval", "1000", "Save the position of clients every x ticks. This is to avoid bots getting stuck in walls during a long playback and lots of jumps.", _, true, 0.0);
	g_hCVRespawnOnDeath = CreateConVar("sm_botmimic_respawnondeath", "0", "Respawn the bot when he dies during playback?", _, true, 0.0, true, 1.0);
...
// The Following Sections have been added by Will Hord
public Action Command_Spawnbot(client,args){
	// CREATE: Should Spawn bot on next round
	//Event_OnPlayerSpawn(Handle:event, name[], dontBroadcast);
  PrintToConsole(client, "Steven is a bitch");
  HookEvent("round_start",Event_OnPlayerSpawn );
  PrintToConsole(client, "Steven's still a bitch");
